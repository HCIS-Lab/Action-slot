...
class TACO(Dataset):
            ...
            for s in tqdm(basic_scenarios, file=sys.stdout):
                ...
                if os.path.isdir(variants_path):
                    ...
                    for v in variants:
                        ...
                        if os.path.isdir(v+"/rgb/downsampled/"):
                            check_data = [v+"/rgb/downsampled/"+ img for img in os.listdir(v+"/rgb/downsampled/") if os.path.isfile(v+"/rgb/downsampled/"+ img)]
                            check_data.sort()
                        else:
                            continue
                        ...
                        for m in range(max_num):
                            ...
                            for i in range(start, end_frame+1, step):
                                ...
                                if os.path.isfile(v+"/rgb/downsampled//"+imgname):
                                    videos_temp.append(v+"/rgb/downsampled//"+imgname)
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len):
            ...
            if self.args.plot:
                data['raw'].append(x)
            if self.args.bg_mask and i%2 == 0:
