...
class ConvGRU(nn.Module):
    ...
    def infer(self, input_tensor, hidden_state=None):
        """
        ...
        return layer_output_list, last_state_list, hidden_state_out
...
class CNN_CONVGRU(nn.Module):
    def __init__(self, num_ego_class, num_actor_class, road):
        super(CNN_CONVGRU, self).__init__()
        ...
        self.convGRU = ConvGRU(input_size=(8, 24),
                input_dim=512,
                hidden_dim=256,
                kernel_size=(3,3),
                num_layers=1,
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = torch.reshape(x, (batch_size, seq_len, 512, height, width))
        _, x = self.convGRU(x)
        x = x[0][0]
